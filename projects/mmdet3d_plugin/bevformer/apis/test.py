...
def custom_multi_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False, occ_threshold=0.25):
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            result, occ_results = model(return_loss=False, rescale=True, occ_threshold=occ_threshold, **data)
            # encode mask results
    ...
    if gpu_collect:
        ...
        if len(occupancy_results):
            ...
            occupancy_results = collect_results_cpu(occupancy_results, len(dataset), tmpdir)
        else:
            occupancy_results = None

    # if mask_results is None:
    #     return bbox_results
    return {'bbox_results': bbox_results, 'mask_results': mask_results, 'occupancy_results': occupancy_results}
