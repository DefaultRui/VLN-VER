...
class VoxelFormerEncoder(TransformerLayerSequence):
    ...
    def __init__(self, *args, pc_range=None, num_points_in_pillar=None, num_points_in_voxel=1,
    ...
    def get_reference_points(bev_z, bev_h, bev_w, num_points_in_voxel=1, dim='3d', bs=1, device='cuda', dtype=torch.float):
        ...
        if dim == '3d':
            ...
            ys = torch.linspace(0.5, bev_h - 0.5, bev_h, dtype=dtype,
                                device=device).view(1, bev_h, 1).expand(1, bev_z, bev_h, bev_w) / bev_h
            xs = torch.linspace(0.5, bev_w - 0.5, bev_w, dtype=dtype,
                                device=device).view(1, 1, bev_w).expand(1, bev_z, bev_h, bev_w) / bev_w
            ref_3d = torch.stack((xs, ys, zs), -1)
            # [4, 150, 150, 3]
            ref_3d = ref_3d.permute(0, 4, 1, 2, 3).flatten(2).permute(0, 2, 1)
            # [4, 150, 150, 3] -> [4, 3, 150, 150] -> [4, 3, 22500] -> [4, 22500, 3]
    ...
    def point_sampling(self, reference_points, pc_range,  img_metas):
        ...
        with open('path to/camera_parameters/world2pixel/' + scan + '.json', 'r') as f1:
            data1 = json.load(f1)
        for deg in range(6):
        ...
        #     for deg in range(6):
        #         datakey = vp + '_i' + str(angle) + '_' + str(deg)
        #         lidar2img.append(data1[datakey])
        # for img_meta in img_metas:
        #     lidar2img.append(img_meta['lidar2img'])  # [6 * [4*4 matrix]]
        with open('path to/scanvp2cord.pkl','rb') as f2:
        ...
        # [1, 4, 22500, 4] for matrix mul!!

        reference_points = reference_points.permute(1, 0, 2, 3) # [4, 1, 22500, 4]
        D, B, num_query = reference_points.size()[:3] # D -> num_points_in_pillar
        ...
        bev_mask = (reference_points_cam[..., 2:3] > eps) # [4, 1, 6, 22500, 1]
        reference_points_cam = reference_points_cam[..., 0:2] / torch.maximum(
            reference_points_cam[..., 2:3], torch.ones_like(reference_points_cam[..., 2:3]) * eps)
        # # [4, 1, 6, 22500, 2]

        # reference_points_cam[..., 0] /= img_metas[0]['img_shape'][0][1] # 1280
        # reference_points_cam[..., 1] /= img_metas[0]['img_shape'][0][0] # 736
        ...
        bev_mask = bev_mask.permute(2, 1, 3, 0, 4).squeeze(-1) # [6, 1, 22500, 4, 1] -. [6, 1, 22500, 4,]
    ...
    def forward(self,
        ...
        # kwargs['img_metas'] = ['filename', 'ori_shape', 'img_shape', 'lidar2img', 'pad_shape', 'scale_factor', 'box_mode_3d', 'box_type_3d',\
        #  'img_norm_cfg', 'sample_idx', 'prev_idx', 'next_idx', 'pts_filename', 'scene_token', 'can_bus', 'prev_bev_exists'])
        # [6, 1, 22500, 4, 2]; [6, 1, 22500, 4, 2]

        # bug: this code should be 'shift_ref_2d = ref_2d.clone()', we keep this bug for reproducing our results in paper.
        # shift_ref_2d = ref_2d  # .clone()
        shift_ref_2d = ref_2d.clone()
        # import ipdb;ipdb.set_trace()
        shift_ref_2d += shift[:, None, None, :] # [1, 22500, 1, 3] + [1, 1, 1, 3]
        ...
        if prev_bev is not None:
            prev_bev = prev_bev.permute(1, 0, 2) # # [1, 22500, 256]
            ...
            hybird_ref_2d = torch.stack([ref_2d, ref_2d], 1).reshape(
                bs*2, len_bev, num_bev_level, 3) # [2, 22500, 1, 2]
        ...
        for lid, layer in enumerate(self.layers):
            output = layer(
                ...
                *args,
                ...
                bev_mask=bev_mask, # [6, 1, 22500, 4, 4]
                prev_bev=prev_bev, # [2, 22500, 256] or None
                **kwargs)
