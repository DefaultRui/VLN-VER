...
class VoxelPerceptionTransformer(BaseModule):
    ...
    def init_layers(self):
        ...
        if self.decoder is not None and self.decoder_on_bev and self.voxel_2_bev_type == 'mlp':
    ...
    def get_voxel_features(
        ...
        shift = bev_queries.new_tensor([[0., 0., 0.]])
        ...
        # try:
        #     feat = mlvl_feats.reshape(18, 1, 14, 14, 768).permute(1, 0, 4, 2, 3) # -> [1, 6, 768, 14, 14]
        # except:
        #     import ipdb;ipdb.set_trace()
    ...
    def forward(self,
        ...
        voxel_embed = voxel_embed.permute(1, 0, 2)
        ...
        if self.decoder_on_bev:
            if self.voxel_2_bev_type == 'mlp':
                ...
                bev_embed = torch.max(bev_embed, dim=-1)[0] # (p1 + p2 + ... + pb, out_channels)
