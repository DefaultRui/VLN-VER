...
def generate_the_ego_car():
    ego_range = [-0.5, -0.5, -1.2, 0.5, 0.5, 0.3]
...
def visualize_occ(points, labels, ego_dict):
    ...
    colors_map = np.array(
        [   
            [0, 0, 0, 255],  # space
            [112, 128, 144, 30],  #  1 wall  Slategrey [112, 128, 144, 255]
            [0, 207, 191, 155],  # 2  floor  nuTonomy green  
            [255, 158, 0, 255],  # 3 chair  orange
            [0, 0, 230, 255],  #  4 door  Blue
            [255, 99, 71, 255],  #  5 table  Tomato
            [222, 184, 135, 255], # 6 objects Burlywood 
            [233, 150, 70, 255],  # 7 cabinet  Darksalmon
            [47, 79, 79, 255],  #  8 window  Darkslategrey
            [255, 140, 0, 255],  #  9 sofa  Darkorange
            [255, 61, 99, 255],  #  10 bed  Red
            [0, 175, 0, 255],  # 11 plant  Green
            [255, 69, 0, 255],  # 12 sink  Orangered
            [175, 0, 75, 255],  # 13 stairs
            [255,255, 255, 10],  #  14 ceiling  [75, 0, 75, 255]
            [0, 0, 230, 255],  #  15 shelving+railing  Blue
    ...
    figure = mlab.figure(size=(500, 500), bgcolor=(1, 1, 1))
...
if __name__ == '__main__':
    # occ_path = '/data1/liurui_data/script_for_mp3d/occupancy/gt_occ_size_new/0.1/2t7WUuJeko7/ee3c278732514701bacdfdacf81f4915.npy'  # TODO set occupancy path
    occ_path = '/data1/liurui_data/script_for_mp3d/occupancy/gt_occ_size_new/0.05/zsNo4HB9uLZ/0753202108e24c0094f09c60b8f36127.npy'  # TODO set occupancy path
    # default setting of the data info
    num_classes = 16
    point_cloud_range = [-6.0, -6.0, -1.5, 6.0, 6.0, 2.0]
    # occupancy_size = [0.1, 0.1, 0.1]
    occupancy_size = [0.05, 0.05, 0.05]
    # voxel_size = 0.1
    voxel_size = 0.05
    add_ego_car = True
    ...
    image = visualize_occ(points, labels, ego_dict)
    plt.savefig('/data1/liurui_data/script_for_mp3d/occupancy/tmp/' + \
                occ_path.split('/')[-2] + occ_path.split('/')[-1].split('.')[0] + '.png')
